...
export class AuthService {
  ...
  /**
   * Vérifie si l'utilisateur est authentifié, tente un refresh si nécessaire
   */
  async ensureAuthenticated(): Promise<boolean> {
    // 1. Vérifier si on a un token valide en mémoire
    if (this.store.isAuthenticated()) {
      return true;
    }

    try {
      const result = await firstValueFrom(this.refresh());
      return true;
    } catch (error) {
      return false;
    }
  ...
  getUserInfo(): Observable<UserInfo> {
    return this.http
      .get<UserInfo>(`${SSO_API_URL}/api/Auth/userinfo`, { withCredentials: true })
      .pipe(
        catchError((err) => {
          return throwError(() => err);
        }),
  ...
  logout(): Observable<{ message: string }> {
    return this.http
      .post<{ message: string }>(`${SSO_API_URL}/api/Auth/logout`, {}, { withCredentials: true })
      .pipe(
        tap(() => {
          this.store.clear();
