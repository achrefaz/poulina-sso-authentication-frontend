...
export class AuthService {
  ...
  async redirectToSso(): Promise<void> {
    ...
    // console.log('Stocké verifier:', sessionStorage.getItem(KEY_VERIFIER));
  ...
  refresh(): Observable<RefreshResponse> {
    return this.http.post<RefreshResponse>(`${this.api}/api/Auth/refresh`, {}).pipe(
  ...
  getUserInfo(): Observable<UserInfo> {
    return this.http
      .get<UserInfo>(`${this.api}/api/Auth/userinfo`)
      .pipe(catchError((err) => throwError(() => err)));
  }


  logout(): Observable<{ message: string }> {
    return this.http.post<{ message: string }>(`${this.api}/api/Auth/logout`, {}).pipe(
